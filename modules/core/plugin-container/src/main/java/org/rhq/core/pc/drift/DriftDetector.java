...
public class DriftDetector implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            DriftDetectionStrategy detectionStrategy = null;
            ResourceContainer rc = inventoryManager.getResourceContainer(schedule.getResourceId());
            if (rc.getResourceComponent() instanceof DriftDetectionFacet) {
                try {
                    ...
                    detectionStrategy = new FacetBasedDriftDetectionStrategy(facet, driftClient, changeSetMgr);
                } catch (PluginContainerException e) {
                    if (log.isDebugEnabled()) {
                        log.debug("Failed to get the component of the resource with id " + schedule.getResourceId() + ", while trying to collect drift for " + schedule + ". The drift detection won't occur.", e);
                    }
                    return;
                }
            } else {
                detectionStrategy = new FileBasedDriftDetectionStrategy(driftClient, changeSetMgr);
            }
            ...
            try {
                if (changeSetMgr.changeSetExists(schedule.getResourceId(), createHeaders(schedule, COVERAGE, 0))) {
                    ...
                    detectionStrategy.generateDriftChangeSet(detectionSummary);
                } else {
                    detectionSummary.setType(COVERAGE);
                    detectionStrategy.generateSnapshot(detectionSummary);
                }
